...
public final class TestTimeSource extends AbstractLongTimeSource {
    ...
    public final void m1749plusAssignLRDsOJo(long duration) {
        long j = duration;
        long longDelta = Duration.m1659toLongimpl(j, getUnit());
        boolean z = true;
        if ((((longDelta - 1) | 1) == Long.MAX_VALUE ? 1 : 0) == 0) {
            ...
            if ((this.reading ^ longDelta) >= 0 && (this.reading ^ newReading) < 0) {
                m1748overflowLRDsOJo(duration);
            }
            this.reading = newReading;
            return;
        }
        long half = Duration.m1616divUwyO8pc(j, 2);
        if (((Duration.m1659toLongimpl(half, getUnit()) - 1) | 1) != Long.MAX_VALUE) {
            z = false;
        }
        if (!z) {
            ...
            try {
                ...
                m1749plusAssignLRDsOJo(Duration.m1648minusLRDsOJo(j, half));
    ...
    private final void m1748overflowLRDsOJo(long duration) {
        throw new IllegalStateException("TestTimeSource will overflow if its reading " + this.reading + DurationUnitKt.shortName(getUnit()) + " is advanced by " + Duration.m1662toStringimpl(duration) + '.');
    }
}
