...
public class TransitionSet extends Transition {
    ...
    public void setCurrentPlayTimeMillis(long playTimeMillis, long lastPlayTimeMillis) {
        ...
        boolean isReverse = j < j2;
        if ((j >= 0 && j2 < 0) || (j <= duration && j2 > duration)) {
            this.mEnded = false;
        ...
        if (this.mPlayTogether) {
            ...
            int startIndex = indexOfTransition(j2);
            if (j >= j2) {
                int i2 = startIndex;
                while (true) {
                    ...
                    transition.setCurrentPlayTimeMillis(playTime, j2 - transitionStart);
                    i2 = i4 + 1;
                    j3 = 0;
                }
            } else {
                int i5 = startIndex;
                while (i5 >= 0) {
                    Transition transition2 = this.mTransitions.get(i5);
                    long transitionStart2 = transition2.mSeekOffsetInParent;
                    long playTime2 = j - transitionStart2;
                    transition2.setCurrentPlayTimeMillis(playTime2, j2 - transitionStart2);
                    if (playTime2 >= 0) {
                        break;
                    }
                    i5--;
        ...
        if ((playTimeMillis > duration && j2 <= duration) || (playTimeMillis < 0 && j2 >= 0)) {
