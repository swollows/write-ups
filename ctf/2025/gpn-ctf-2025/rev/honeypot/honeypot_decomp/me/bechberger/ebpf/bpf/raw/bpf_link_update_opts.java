...
public class bpf_link_update_opts {
    private static final GroupLayout $LAYOUT = MemoryLayout.structLayout((MemoryLayout[])new MemoryLayout[]{Lib.C_LONG.withName("sz"), Lib.C_INT.withName("flags"), Lib.C_INT.withName("old_prog_fd"), Lib.C_INT.withName("old_map_fd"), MemoryLayout.paddingLayout((long)4L)}).withName("bpf_link_update_opts");
    ...
    private static final ValueLayout.OfInt old_prog_fd$LAYOUT = (ValueLayout.OfInt)$LAYOUT.select(new MemoryLayout.PathElement[]{MemoryLayout.PathElement.groupElement((String)"old_prog_fd")});
    private static final long old_prog_fd$OFFSET = 12L;
    private static final ValueLayout.OfInt old_map_fd$LAYOUT = (ValueLayout.OfInt)$LAYOUT.select(new MemoryLayout.PathElement[]{MemoryLayout.PathElement.groupElement((String)"old_map_fd")});
    private static final long old_map_fd$OFFSET = 16L;
    ...
    public static int old_map_fd(MemorySegment struct) {
        return struct.get(old_map_fd$LAYOUT, 16L);
    }

    public static void old_map_fd(MemorySegment struct, int fieldValue) {
        struct.set(old_map_fd$LAYOUT, 16L, fieldValue);
    }

    public static MemorySegment asSlice(MemorySegment array, long index2) {
        return array.asSlice(bpf_link_update_opts.layout().byteSize() * index2);
    }

    public static long sizeof() {
        return bpf_link_update_opts.layout().byteSize();
    }

    public static MemorySegment allocate(SegmentAllocator allocator) {
        return allocator.allocate((MemoryLayout)bpf_link_update_opts.layout());
    }

    public static MemorySegment allocateArray(long elementCount, SegmentAllocator allocator) {
        return allocator.allocate((MemoryLayout)MemoryLayout.sequenceLayout((long)elementCount, (MemoryLayout)bpf_link_update_opts.layout()));
    }

    public static MemorySegment reinterpret(MemorySegment addr2, Arena arena, Consumer<MemorySegment> cleanup) {
        return bpf_link_update_opts.reinterpret(addr2, 1L, arena, cleanup);
    }

    public static MemorySegment reinterpret(MemorySegment addr2, long elementCount, Arena arena, Consumer<MemorySegment> cleanup) {
        return addr2.reinterpret(bpf_link_update_opts.layout().byteSize() * elementCount, arena, cleanup);
    }
}
