...
import me.bechberger.ebpf.bpf.raw.__pthread_cond_s;

public class pthread_cond_t {
    private static final GroupLayout $LAYOUT = MemoryLayout.unionLayout((MemoryLayout[])new MemoryLayout[]{__pthread_cond_s.layout().withName("__data"), MemoryLayout.sequenceLayout((long)48L, (MemoryLayout)Lib.C_CHAR).withName("__size"), Lib.C_LONG_LONG.withName("__align")}).withName("$anon$75:9");
    ...
    private static long[] __size$DIMS = new long[]{48L};
    ...
    public static MemorySegment asSlice(MemorySegment array, long index2) {
        return array.asSlice(pthread_cond_t.layout().byteSize() * index2);
    }

    public static long sizeof() {
        return pthread_cond_t.layout().byteSize();
    }

    public static MemorySegment allocate(SegmentAllocator allocator) {
        return allocator.allocate((MemoryLayout)pthread_cond_t.layout());
    }

    public static MemorySegment allocateArray(long elementCount, SegmentAllocator allocator) {
        return allocator.allocate((MemoryLayout)MemoryLayout.sequenceLayout((long)elementCount, (MemoryLayout)pthread_cond_t.layout()));
    }

    public static MemorySegment reinterpret(MemorySegment addr2, Arena arena, Consumer<MemorySegment> cleanup) {
        return pthread_cond_t.reinterpret(addr2, 1L, arena, cleanup);
    }

    public static MemorySegment reinterpret(MemorySegment addr2, long elementCount, Arena arena, Consumer<MemorySegment> cleanup) {
        return addr2.reinterpret(pthread_cond_t.layout().byteSize() * elementCount, arena, cleanup);
    }
}
