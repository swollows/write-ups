				...
				if (media_info.status == "ok" && media_info.errno == null){
					...
					for (var i = 0; i < media_info.count; i++){
						...
						str += "<tr onMouseOver=\"this.style.background='#D8D8D8'\" onMouseOut=\"this.style.background=''\">"
							 ...
							 + "<a rel=\"image1\" href=\"/dws/api/GetFile?id=" + storage_user.get("id") + "&tok=" +storage_user.get("tok")+"&volid="+obj.volid+"&path="+encodeURIComponent(obj.path)+"&filename="+encodeURIComponent(obj.name)+"\" title=\"" + obj.name + "\">"
							 + "<div>"
