...
Page.prototype =
{
	...
	InitValue: function(xml)
	{
		...
		if(OBJ("v4addr").value==="Oray.cn")//kity
		{
	...
	PreSubmit: function()
	{
		...
		if(OBJ("en_ddns").checked)
		{
			...
			if(COMM_EatAllSpace(OBJ("host").value) == "" && OBJ("v4addr").value!="Oray.cn")
			{			
	...
	UpdatnowOray: function()//kity
	{
		...
		if(status=="successed")
		{
			...
			OBJ("peanut_status").innerHTML="<?echo i18n("Connected");?>";	
		...
		else if(status=="badAuth")
		{
			...
			OBJ("peanut_status").innerHTML="<?echo i18n("Disconnected");?>";
			OBJ("peanut_detail_div").style.display="none";
		}
		else
		{
...
function sync_GetCFG(Cache, Services, Handler)//kity
{
