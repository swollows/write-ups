...
Page.prototype =
{
	...
	*********************************************************/
	...
	InitWANConnMode: function()
	{
		...
		if( this.wanact.svcsname != null )
		{
			...
			else
			{
				...
				var wanllmode = XG(this.wanllact.inetp+"/ipv6/mode");
				if(wanmode != "" && wanactive !="0")
				{			 
					if(wanllmode == "6IN4")
						COMM_SetSelectValue(OBJ("wan_ipv6_mode"), wanllmode);
					else
	...
	InitWAN6IN4Value: function()
	{
		...
		if(!(wanmode == "6IN4" || wanllmode == "6IN4")) return true;
							
		

		if(wanmode == "6IN4" && wanllmode == "LL")
		{
...
else if(wanmode == "AUTO" && wanllmode == "6IN4")
{
		
		OBJ("w_tu_lov6_ipaddr").value	= XG(this.wanllact.inetp+"/ipv6/ipaddr");
		OBJ("w_tu_pl").value		= XG(this.wanllact.inetp+"/ipv6/prefix");
		OBJ("w_tu_rev6_ipaddr").value	= XG(this.wanllact.inetp+"/ipv6/gateway");
		OBJ("w_tu_rev4_ipaddr").value	= XG(this.wanllact.inetp+"/ipv6/ipv6in4/remote");
		OBJ("w_st_pdns").value		= XG(this.wanllact.inetp+"/ipv6/dns/entry:1");
		OBJ("w_st_sdns").value		= XG(this.wanllact.inetp+"/ipv6/dns/entry:2");
}			
...
else if(wanmode == "AUTO" && wanllmode == "6IN4")
{
		...
		var count = XG(this.wanllact.inetp+"/ipv6/dns/count");
		if(count > 0)
		{
			OBJ("w_dhcp_dns_auto").checked = false;
			SetRadioValue("w_dhcp_dns_rad", "manual");
			OBJ("w_dhcp_pdns").value		= XG(this.wanllact.inetp+"/ipv6/dns/entry:1");
			if(count > 1)
				OBJ("w_dhcp_sdns").value		= XG(this.wanllact.inetp+"/ipv6/dns/entry:2");
		}
}	

                //+++ Grace Lin, enable dhcp-pd when child is not enpty.
                if(XG(this.wanact.svcs+"/inf/child") != "")
		{		
			OBJ("en_dhcp_pd").checked = true;
			OBJ("l_ipaddr").disabled = true;
...
//+++ Grace				
				if(OBJ("en_dhcp_pd").checked)
				{
					...
					XS(this.wan1.infp+"/infnext", "WAN-3");
					XS(this.wanllact.svcs+"/inf/infprevious", "WAN-1");
					XS(this.wanllact.svcs+"/inf/infnext", "WAN-4");
					XS(this.wanact.svcs+"/inf/infprevious", "WAN-3");  
					XS(this.wanact.svcs+"/inf/child", "LAN-4");
					XS(this.wanact.svcs+"/inf/childgz", "LAN-5");
          XS(this.wanllact.inetp+"/addrtype", "ipv6");
					...
					XS(this.wanact.svcs+"/inf/active", "1");
					XS(this.wanllact.inetp+"/ipv6/mode", "6IN4");
				}
				else
				{
				...
				XS(this.wanact.svcs+"/inf/infprevious", "WAN-1");
					...
					//+++ Grace Lin, Set the infnext of WAN-4 is LAN-4
					//               in order to use the specified DNS servers.
					//XS(this.wanact.svcs+"/inf/infnext", "");
					XS(this.wanact.svcs+"/inf/infnext", "LAN-4");  
...
//+++ Grace
				if(OBJ("en_dhcp_pd").checked)
				{
						...
						/* ipaddr, prefix */
					XS(this.wanllact.inetp+"/ipv6/ipaddr", OBJ("w_tu_lov6_ipaddr").value);
					XS(this.wanllact.inetp+"/ipv6/prefix", OBJ("w_tu_pl").value);
	
					/* gateway */
					XS(this.wanllact.svcs+"/inf/defaultroute", "1");	
					XS(this.wanllact.inetp+"/ipv6/gateway", OBJ("w_tu_rev6_ipaddr").value);
	
					/* dns */
					var dnscnt = 0;
					if (OBJ("w_dhcp_dns_auto").checked)
					{
						XS(this.wanllact.inetp+"/ipv6/dns/entry:1","");
						XS(this.wanllact.inetp+"/ipv6/dns/entry:2","");
					}
					else
					{
						if(OBJ("w_dhcp_pdns").value != "")
						{
							XS(this.wanllact.inetp+"/ipv6/dns/entry:1", OBJ("w_dhcp_pdns").value);
							dnscnt+=1;
						}
						if(OBJ("w_dhcp_sdns").value != "")
						{
							XS(this.wanllact.inetp+"/ipv6/dns/entry:2", OBJ("w_dhcp_sdns").value);
							dnscnt+=1;
						}
					}
						XS(this.wanllact.inetp+"/ipv6/dns/count", dnscnt);
	
	                                /* set ipv4 address for server */
					XS(this.wanllact.inetp+"/ipv6/ipv6in4/remote", OBJ("w_tu_rev4_ipaddr").value);
