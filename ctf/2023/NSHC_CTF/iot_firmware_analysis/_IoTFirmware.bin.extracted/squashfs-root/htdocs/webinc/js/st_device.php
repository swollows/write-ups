	...
	InitWLAN: function ( wlan_phyinf, wifi_phyinf )
	{
        ...
        switch (IEEE80211mode)
		{
		   ...
		   case "an":
				...
				break;
		   case "acna":
		   		OBJ("st_80211mode"+str_Aband).innerHTML  = "<?echo i18n("Mixed 802.11ac, 802.11n and 802.11a");?>";
                check_bandwidth = 1;
		   break;
		   case "acn":
		   		OBJ("st_80211mode"+str_Aband).innerHTML  = "<?echo i18n("Mixed 802.11ac and 802.11n");?>";
                check_bandwidth = 1;
		   break;
		   case "ac":
		   		OBJ("st_80211mode"+str_Aband).innerHTML  = "<?echo i18n("802.11ac only");?>";
                check_bandwidth = 1;
				break;
		   case "n":
		...
		if (XG  (this.waninetp+"/addrtype") == "ipv4")
		{
			...
			else
			{
				if(XG  ( this.waninetp+"/ipv4/static")== "1")
				{
			    		...
			    		OBJ("st_wantype").innerHTML  = "<?echo i18n("Static IP");?>";
				...
				else
				{
			    		...
			    		OBJ("st_wantype").innerHTML  = "<?echo i18n("DHCP Client");?>";
					if ((XG  (this.rwaninetp+"/ipv4/valid")== "1")&& (wancable_status==1))
					{
						wan_network_status=1;
