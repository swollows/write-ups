...
{
	<?
		if(isfile("/etc/services/UPNPC.php")==1) echo 'services: "WEBACCESS,INET.LAN-1,DDNS4.WAN-1,UPNPC,SAMBA",';
		else echo 'services: "WEBACCESS,INET.LAN-1,DDNS4.WAN-1,SAMBA",';
	...
	CheckAccessPort: function()
	{
		...
		if (!TEMP_IsDigit(OBJ("webaccess_httpport").value) || OBJ("webaccess_httpport").value==23  || OBJ("webaccess_httpport").value==80 || OBJ("webaccess_httpport").value==443 || OBJ("webaccess_httpport").value==21 || OBJ("webaccess_httpport").value==143)
		{
			BODY.ShowAlert("<?echo i18n("The HTTP Access Port number is not valid.");?>");
			return false;
		}
		if (!TEMP_IsDigit(OBJ("webaccess_httpsport").value) || OBJ("webaccess_httpsport").value==80 || OBJ("webaccess_httpsport").value==443 || OBJ("webaccess_httpsport").value==21 || OBJ("webaccess_httpsport").value==143)
		{
	...
	{			
		...
		for(var i=0;i < passwd.length;i++)
			...
			//==avoid holomorphic word==
			if(passwd.match(/[\x00-\x1F\x80-\xFF]/g))
			{
				BODY.ShowAlert("<?echo I18N("j","Invalid Password.");?>");
				return false;
			}
			var new_user = true;
			for(var i=0;i < this.access_rule.length;i++)
