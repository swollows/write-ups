...
?>
	...
	InitWAN: function ()
	{
		...
		else if ((XG(this.waninetp+"/addrtype") == "ipv6")&& wancable_status==1)
		{
			...
			// 6rd case, but assigned prefix len is 64, all ips are assigned to lan
			if(str_wantype == "6RD")
			{
				if ( str_wanipaddr == "")
				{
			...
			if(str_wantype == "AUTO")
			{
				if(XG(wan3inetp+"/ipv6/mode") == "6IN4")
				{
					var str_wanipaddr = XG(rwan3+"/ipv6/ipaddr");
					var str_wanprefix = "/"+XG(rwan3+"/ipv6/prefix");
					var str_wangateway = XG(rwan3+"/ipv6/gateway");
					var str_wanDNSserver  = XG(rwan3+"/ipv6/dns:1");
					var str_wanDNSserver2 = XG(rwan3+"/ipv6/dns:2");
					
					var wan_uptime = S2I(XG  (rwan3+"/uptime"));
					var wan_delta_uptime = (system_uptime-wan_uptime);
				}
				else
				{	
		...
		else
		{
			...
			else if(str_wantype=="AUTO")
			{
				...
				var rwan3 = PXML.FindModule("RUNTIME.INF.WAN-3");
				var rwan3mode = XG(rwan3+"/runtime/inf/inet/ipv6/mode");
				//if(rwanmode=="STATEFUL") 		str_wantype = "DHCPv6";
				//else if(rwanmode=="STATELESS") 	str_wantype = "SLAAC";
				//else 							str_wantype = "Autoconfiguration";
				if(rwan3mode=="6IN4")	str_wantype = "6IN4";
				else	str_wantype = "Autoconfiguration (SLAAC/DHCPV6)";
