...
Page.prototype =
{
	...
	OnClickQOSEnable: function()
	{
		...
		if (this.feature_china && XG(bwc+"/bwc:2/entry:1/enable")==="1")
		{
			if (!confirm('<?echo I18N("j","The Traffic Control is already enabled, and this two function are the same. Do you want to disable Traffic Control to use QoS Engine?");?>')) 
			{	
				OBJ("en_qos").checked = false;
				OBJ("en_qos").disabled = true;
			}
			else XS(bwc+"/bwc:2/entry:1/enable", 0)
		}
	...
	IPRangeCheck: function(i)
	{
		...
		if(OBJ("dst_startip_"+i).value !== "")
		{
			...
			if(TEMP_CheckNetworkAddr(OBJ("dst_startip_"+i).value) || (router_mode === "1W2L" && TEMP_CheckNetworkAddr(OBJ("dst_startip_"+i).value, lan2ip, lan2mask)))
			{	
				alert("<?echo I18N("j", "IP address should not be in LAN subnet.");?>");
				OBJ("dst_startip_"+i).focus();
				return false;
			}				
		}
		if(OBJ("dst_endip_"+i).value !== "")
		{
			...
			if(TEMP_CheckNetworkAddr(OBJ("dst_endip_"+i).value) || (router_mode === "1W2L" && TEMP_CheckNetworkAddr(OBJ("dst_endip_"+i).value, lan2ip, lan2mask)))
			{	
				alert("<?echo I18N("j", "IP address should not be in LAN subnet.");?>");
				OBJ("dst_endip_"+i).focus();
				return false;
