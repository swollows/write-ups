...
{
	<?
		if($FEATURE_CHINA_SPECIAL_WAN == "1") echo 'services: "DEVICE.LAYOUT,DEVICE.HOSTNAME,PHYINF.WAN-1,INET.BRIDGE-1,INET.INF,WAN,RUNTIME.INF.WAN-3,INET.LAN-4,RUNTIME.INF.WAN-1,RUNTIME.INF.WAN-4,DHCPC4.WAN,STARSPEED.WAN-1,NETSNIPER.NAT-1,REBOOT",';
		else  echo 'services: "DEVICE.LAYOUT,DEVICE.HOSTNAME,PHYINF.WAN-1,INET.BRIDGE-1,INET.INF,WAN,RUNTIME.INF.WAN-3,INET.LAN-4,RUNTIME.INF.WAN-1,RUNTIME.INF.WAN-4,DHCPC4.WAN,REBOOT",';
	?>
	OnLoad: function() {},
	OnUnload: function() {},
	dhcpc4: null,
	...
	InitValue: function(xml)
	{
		...
		if (this.special_wan) /* special wan for China, NetSniper*/
		{
			this.nat1 = PXML.FindModule("NETSNIPER.NAT-1");
			if (!this.nat1) { alert("Init NAT ERROR !"); return false; }
		}
	...
	PreSubmit: function()
	{
		...
		if (this.special_wan) PXML.IgnoreModule("STARSPEED.WAN-1");
		...
		if (this.special_wan) /* clear NAT-1 NetSniper */
		{
			XS(this.nat1+"/nat/entry/netsniper/enable", "");
		}
		...
		case "pppoe":
			...
			{
				/* NetSniper */
				XS(this.wan1.inetp+"/ppp4/pppoe/netsniper/enable", OBJ("en_netsniper").checked ? "1" : "0");
				XS(this.nat1+"/nat/entry/netsniper/enable", OBJ("en_netsniper").checked ? "1" : "0");
				
				/* Starspeed */
				XS(this.wan1.inetp+"/ppp4/pppoe/starspeed/enable", OBJ("en_starspeed").checked ? "1" : "0");
				XS(this.wan1.inetp+"/ppp4/pppoe/starspeed/mode", "auto");
				
				/* PPPoE+ */
				XS(this.wan1.inetp+"/ppp4/pppoe/pppoeplus/enable", OBJ("en_pppoeplus").checked ? "1" : "0");
			}
			break;
		case "r_pptp":
		...
		{
			/* NetSniper */
			PXML.CheckModule("NETSNIPER.NAT-1", null, null, "ignore");
			/* Starspeed, PPPoE+*/
			if (OBJ("en_starspeed").checked || OBJ("en_pppoeplus").checked) PXML.CheckModule("STARSPEED.WAN-1", "ignore", "ignore", null);
		}
		return PXML.doc;
	},
	IsDirty: null,
	...
	ori_wan_mode: null,	
	dslite: <?
		if($FEATURE_NODSLITE!="1") echo "true";
		else echo "false";
	?>,
	/* for China special wan: NetSniper, Starspeed, PPPoE+ */
	special_wan: <?
		if($FEATURE_CHINA_SPECIAL_WAN=="1") echo "true";
		else echo "false";
	?>,
	...
	InitPpp4Value: function()
	{
		...
		if (this.special_wan)
		{
			var netsniper_en = XG(this.wan1.inetp+"/ppp4/pppoe/netsniper/enable");
			OBJ("en_netsniper").checked = netsniper_en==1 ? true : false;
			
			var starspeed_en = XG(this.wan1.inetp+"/ppp4/pppoe/starspeed/enable");
			OBJ("en_starspeed").checked	= starspeed_en==1 ? true : false;
			
			var pppoeplus_en = XG(this.wan1.inetp+"/ppp4/pppoe/pppoeplus/enable");
			OBJ("en_pppoeplus").checked = pppoeplus_en==1 ? true : false;
			
			this.OnClickSpecialWan();
		}
	...
	OnChangeWanIpMode: function()
	{
		...
		if (this.special_wan)
		{
			OBJ("show_netsniper").style.display = "none";
			OBJ("show_starspeed").style.display= "none";
			OBJ("show_pppoeplus").style.display = "none";
		}
		//for dslite
		...
		*/
		if (this.dslite) AddItemFromSelect(OBJ("wan_ip_mode"),"DS-Lite","dslite");
		...
		case "pppoe":
			...
			if (this.special_wan)
			{
				OBJ("show_netsniper").style.display = (OBJ("wan_ip_mode").value === "pppoe") ? "block" : "none";
				OBJ("show_starspeed").style.display		= (OBJ("wan_ip_mode").value === "pppoe") ? "block" : "none";
				OBJ("show_pppoeplus").style.display = (OBJ("wan_ip_mode").value === "pppoe") ? "block" : "none";
				this.OnClickSpecialWan();
			}
	...
	SpecialWanDisable: function(sniper, starspeed, pppoeplus)
	{
		OBJ("en_netsniper").disabled = sniper;
		OBJ("en_starspeed").disabled = starspeed;
		OBJ("en_pppoeplus").disabled = pppoeplus;
	},
	OnClickSpecialWan: function()
	{
		if(OBJ("en_netsniper").checked)
			this.SpecialWanDisable(false, true, true);
		else if(OBJ("en_starspeed").checked)
			this.SpecialWanDisable(true, false, true);
		else if(OBJ("en_pppoeplus").checked)
			this.SpecialWanDisable(true, true, false);
		else
			this.SpecialWanDisable(false, false, false);
	},
	DisableDNS: function()
	{
