...
function inet_ipv6_6in4($mode, $inf, $infp, $stsp, $inetp)
{
	...
	else if ($mode=="6RD")
	{
		...
		if ($child!="")
		{
			/* Alpha, HuanYao Kang, Fix 6rd prefix length bug 
	...
	if($infprev!="")
	{
		...
		if($infprev == "WAN-3")
		{
				...
				if(strstr($prevdevnam,"sit")!="")
				{
					...
					$prev_infp = XNODE_getpathbytarget("", "inf", "uid", $infprev, 0);
					$prev_inet = query($prev_infp."/inet");	
					$prev_inetp = XNODE_getpathbytarget("/inet", "entry", "uid", $prev_inet, 0);
					$router = query($prev_inetp."/ipv6/gateway");	
					...
					startcmd("dhcp6c -c ".$cfg." -p ".$pid." -t LL -o ".$eth." ".$dname);
					return 0;
...
else if	($mode=="AUTO")	inet_ipv6_auto($inf, $infp, $ifname, $phyinf, $stsp, $inetp);	//Autoconfiguration or 6in4 + enable DHCP-PD
...
else if	($mode=="6IN4"	||	 $mode=="6TO4"	||	 $mode=="6RD")	inet_ipv6_6in4($mode, $inf, $infp, $stsp, $inetp);
