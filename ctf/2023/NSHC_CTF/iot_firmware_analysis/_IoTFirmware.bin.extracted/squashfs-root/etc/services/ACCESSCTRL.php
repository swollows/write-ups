...
if(query("/acl/accessctrl/enable")=="1")
{
			...
			if(query("webfilter/enable")=="1")
			{
				...
				{	
					...
					if($webf_policy=="DROP")
					{
						...
						fwrite("a",$START, $iptcmd.$urlcmd." -j HIJACK --to-url ".$lanip."/web_filter_info.php\n");
					}
					else 
				...
				{
					if($lanip!="")
						fwrite("a",$START, $iptcmd." -p tcp --dport 80 -m string --http_req -j HIJACK --to-url ".$lanip."/web_filter_info.php\n");
					else
	...
	if ($usehttp=="1")
	{
		fwrite("a",$START, "echo 2 > /proc/sys/net/ipv4/netfilter/ip_conntrack_fastnat\n");	  
		fwrite("a",$STOP, "echo 1 > /proc/sys/net/ipv4/netfilter/ip_conntrack_fastnat\n");	  
	}
}
