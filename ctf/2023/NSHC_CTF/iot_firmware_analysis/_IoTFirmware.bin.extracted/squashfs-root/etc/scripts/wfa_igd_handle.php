...
function add_ext_port($EP)
{
  $done = 0;
	...
	if ($done == 0)
	{
		$newentry = XNODE_add_entry("/runtime/webaccess/portmapping", "EPORT");
		anchor($newentry);
		set("externalport", $EP);
		TRACE_debug("wfa_igd_handle.php: add externalport ".$EP." to ".$newentry);			
	}	
}
...
else if($MODE=="DS_IPT")  //add directserver iptable rules
{
    ...
    /* maintain externalport table for UPNP AddPortMapping */
		add_ext_port($E_PORT);
}
